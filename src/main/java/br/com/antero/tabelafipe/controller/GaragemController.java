...
public class GaragemController {
    ...
    public ResponseEntity<VeiculoFavoritoResponseDTO> adicionarFavorito(
        ...
        VeiculoFavoritoResponseDTO resposta = service.salvarFavorito(dados, usuarioLogado);

        var uri = uriBuilder.path("/api/garagem/{id}").buildAndExpand(resposta.idFavorito()).toUri();
        return ResponseEntity.created(uri).body(resposta);
    }
    @GetMapping
    public ResponseEntity<List<VeiculoFavoritoResponseDTO>> listarGaragem(@AuthenticationPrincipal Usuario usuarioLogado) {

        List<VeiculoFavoritoResponseDTO> garagem = service.listarGaragem(usuarioLogado);
